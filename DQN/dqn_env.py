...
# -----------------------------------------------------
        ...
        self.observation_space = Box(low=np.array([-1000, -1000, -1000, -1000, -1000, -1000, -1000, -1000, -1000, -1000, -1000]), high=np.array([1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000]), dtype=np.float64)
    ...
    def reset(self):
        ...
        self.n_r = np.random.randint(300, 310)
        ...
        for i in range(self.n_lc):
            ...
            for piles in info:
                if piles:
                    ...
                    start_soc = 0
                    charging_time = 0
                end_time = start_time + datetime.timedelta(seconds=charging_time)
        ...
        for i in range(self.n_r):
            ...
            for j in range(len(self.l_c)):
                dis = math.sqrt((self.l_q[-1][0] - self.l_c[j][0])**2 + (self.l_q[-1][1] - self.l_c[j][1])**2)
    ...
    def step(self, action):
        ...
        rewards, load, dis = self.rewards(action, arrival_time_list, rest)
        self.dis = dis
        # self.Loads = load
    ...
    def rewards(self, action, arrival_time, weather_rest):
        ...
        # 1. 抵达距离
        dis = math.sqrt((self.l_q[self.STEP][0] - self.l_c[action][0]) ** 2 + (self.l_q[self.STEP][1] - self.l_c[action][1]) ** 2)
        r_dis = - dis/30
        # 2.等待时间奖励+负载奖励
        r_load = 0
        r_waiting = 0
        ...
        # self.r_n_c[action] = 0
            ...
            if n_wait > 9:
                ...
                charging_time = (soc - (self.max_soc - soc1)) / (20*6)
                ...
                # print(wait_info[n_wait][3].timestamp() - datetime.datetime.now().timestamp(), charging_time)
        ...
        # print('r_waiting', r_waiting)
        # print('r_dis', r_dis)
        # print('r_load', r_load)
        # R = r_arrival_time + r_waiting
        R = r_dis + r_waiting
        return R, r_load, dis

    def history(self):
        # print(self.waiting_time_his, self.dis/self.speed)
        return self.Loads[1], self.waiting_time_his, self.dis
                ...
                if piles[0] == 1:
                    ...
                    if (piles[3] != 0) & (piles[3] < datetime.datetime.now()):
                        ...
                        if self.c_w_l[stations_id][0] > 0:
                            ...
                            end_time = datetime.datetime.now() + datetime.timedelta(seconds=charging_time)
            ...
            if stations_id == action:
                ...
                for piles in stations:
                    if piles[0] == 0:
                        ...
                        charging_time = (self.max_soc - soc) / 20
                        end_time = datetime.timedelta(seconds=charging_time)
                        # 跟新详细信息列表
    ...
    def choose_nearest(self):
        ...
        for i in range(len(self.l_c)):
            dis = math.sqrt((self.l_q[self.STEP][0] - self.l_c[i][0])**2 + (self.l_q[self.STEP][1] - self.l_c[i][1])**2)
            if dis < min_dis:
                min_dis = dis
                a = i
        return a
    ...
    def distance(self):
        for i in range(self.n_r):
            distance_charge_location = []
            for j in range(len(self.l_c)):
                dis = math.sqrt((self.l_q[i][0] - self.l_c[j][0])**2 + (self.l_q[i][1] - self.l_c[j][1])**2)
                distance_charge_location.append(dis)
            self.dis_list.append(distance_charge_location)

        # 当前时间片段
        time_step_dis = self.dis_list[self.STEP]

        return time_step_dis
